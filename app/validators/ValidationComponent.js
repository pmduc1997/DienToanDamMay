...
import { messages, labels } from "./messages";
...
export default class ValidationComponent extends React.Component {
  constructor(props) {
    super(props);
    this.errors = [];
    this.deviceLocale = AppConfig.defaultLang;
    this.rules = props.rules || rules;
    this.messages = props.messages || messages;
  ...
  _addError(fieldName, rule, value, isFn) {
    const errMsg = this.messages[this.deviceLocale][rule]
      .replace("{label}", labels[this.deviceLocale][fieldName] || fieldName)
      .replace("{value}", value);
